...
const MAX_REQUESTS = 5; // Limite um pouco maior para testes rápidos
  ...
  try {
    ...
    if (now - userRate.timestamp > RATE_LIMIT_WINDOW) {
      ...
      if (userRate.count > MAX_REQUESTS) {
        return NextResponse.json({ error: "Muitas tentativas de teste. Aguarde 1 minuto." }, { status: 429 });
      }
    }
    ...
    const response = await fetch('https://app.amplopay.com/api/v1/gateway/pix/receive', {
      ...
      body: JSON.stringify({
        identifier: `michigun-teste-${Date.now()}`,
        amount: 0.01,
        client: { 
          name: discord, 
          email: email,
    ...
    if (!response.ok) {
      console.error("AmploPay Test Error:", data);
      const errorMessage = data.message || "Falha na comunicação com a AmploPay para teste.";
    ...
    return NextResponse.json({ error: "Erro interno do servidor ao gerar PIX de teste." }, { status: 500 });
  }
}
