...
export default function TestePage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    fetch("/api/pix-teste", {
      ...
      .catch((err) => {
        console.error("Erro ao gerar pix de teste:", err);
        toast.dismiss();
        toast.custom((t) => (
          <AlertBadge
            variant="error"
            icon={AlertCircle}
            label={err.message || "Ocorreu um erro ao gerar o PIX de teste."}
            className={t.visible ? 'animate-in fade-in' : 'animate-out fade-out'}
          />
        ), { duration: 3000 });
        setLoading(false);
          ...
          <div className="flex flex-col gap-8">
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-[#d4af37]/10 border border-[#d4af37]/30 text-[#d4af37] text-xs font-bold uppercase tracking-widest rounded-full mb-6">
                <Crown size={12} /> Teste de Pagamento
              </div>
            ...
            <div className="relative w-full overflow-hidden rounded-2xl bg-[#0a0a0a] border border-[#222] shadow-[0_0_50px_rgba(212,175,55,0.1)]">
              ...
              <div className="p-5 sm:p-8">
                <div className="text-center mb-6 sm:mb-8">
                  <p className="text-xs sm:text-sm text-neutral-400 mb-1 font-medium uppercase tracking-widest">Valor de teste</p>
